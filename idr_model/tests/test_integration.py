...
def test_joule_dissipation_formula19():
    """
    Q(r) = σ_a(r)·|E(r)|² = σ_a·v  (Джоулев нагрев).
...
def test_reproduce_fig2_qualitative():
    ...
    # |H|² должно убывать от стенки к центру (скин-эффект)
    # По принципу максимума: H² ≤ H²(R) везде при RHS ≥ 0
    assert u[-1] >= u[0] - 1e-10, (
        f"|H|²(R)={u[-1]:.6e} < |H|²(0)={u[0]:.6e} — нарушен принцип максимума"
    )
...
def test_high_freq_limit():
    ...
    Система должна воспроизводиться.
...
def test_idr_ionization_regime():
    ...
    находиться в ИДР-режиме:
      ...
      νi_max  >>  Da · λ₁  (минимальный критерий соответствия)
