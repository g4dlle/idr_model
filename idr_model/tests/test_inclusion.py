...
class TestSolverWithInclusion:
    ...
    def test_E_field_zero_at_inclusion_rises_inward(self):
        ...
        v = result["v"]
        # v[0] = 0 (ГУ Дирихле на включении), E² возрастает внутрь.
        # Проверяем: максимум не на включении и не на стенке (как у чистой диффузии),
        # а где-то в середине зазора.
        i_max = int(np.argmax(v))
        assert i_max > 0, "Максимум E² на поверхности включения (v[0] = max)"
        assert v[0] == 0.0 or v[0] < v[len(v) // 2], (
            f"E² не возрастает от включения к середине: "
            f"v[0]={v[0]:.3e}, v[mid]={v[len(v)//2]:.3e}"
        )
